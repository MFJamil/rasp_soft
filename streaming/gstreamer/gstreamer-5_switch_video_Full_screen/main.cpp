...
struct CustomData{
    ...
    gulong slider_update_signal_id; /* Singal ID for the slider update signal */
    GstState state;                 /* Current state of the pipeline */
    gint64 duration;                /* Duration of the clip in Nanoseconds */ 
};
bool timeOutSwitch = false;
...
static void realize_cb(GtkWidget *widget,CustomData *data){
    ...
    if (!gdk_window_ensure_native(window))
        g_error("Could not create native window needed for ");
...
void releaseApp(CustomData * data){
   /* Free resources */
...
static void switchMovie(CustomData *data){
  play_movie("M2",data);
}
...
gboolean key_pressed_cb (GtkWidget *widget, GdkEventKey *event, CustomData *data) {
    //g_print("Handling key pressed : %s ",event->keyval  );
...
int main(int arg, char *argv[]) {
    ...
    if (arg>1) timeOutSwitch=(strcmp(argv[1],"timeout")==0);
    /* Initialize GTK */
    gtk_init(&arg,&argv);
